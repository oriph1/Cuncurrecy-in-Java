...
class PlayerTest {
    ...
    @Test
    void isItRemovingAction(){
        player.placeAction(3);
        assertTrue(player.removingSlot(3));
        assertFalse(player.removingSlot(2));
    }

    @Test
    void tokenRemoved(){
        ArrayList<Integer> playerTokens = player.getTokens();
        playerTokens.add(1);
        player.removeToken(1);
        assertTrue(playerTokens.size()==0);
    }

    @Test
    void isTerminated(){
        boolean terminatePre=player.getTerminate();
        player.terminate();
        assertNotEquals(terminatePre,player.getTerminate());
    }
}